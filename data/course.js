const courseData = {
  books: [
    ...
    },
    {
      ...
      lessons: [
        ...
        { id: 27, number: '③', title: '第23课 对应', description: '新标日第23课', xp: 55, type: 'dialogue' },
  ...
  getQuestionsByType: function(type) {
    return this.questionTypes[type] || this.questionTypes.dialogue;
  },
  ...
  getAllLessons: function() {
    ...
    this.books.forEach(book => {
      book.lessons.forEach(lesson => {
        all.push({ ...lesson, bookId: book.id, bookName: book.name });
      });
    });
    return all;
  },
    ...
    for (let book of this.books) {
      for (let lesson of book.lessons) {
        if (lesson.id === lessonId) {
          return { ...lesson, bookId: book.id, bookName: book.name, book: book };
        }
      }
    }
    return null;
